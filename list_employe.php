...
<body class='relative'>
    ...
    <?php
         $sqlEmployes = $db->prepare("SELECT u.id AS idUser, u.last_name AS lastName, u.first_name AS firstName, u.phone as phone, u.email AS email, u.adress AS adress, uc.id AS idCategory, uc.label AS nameCategory FROM user u INNER JOIN user_category uc ON u.user_category_id = uc.id  WHERE u.id = 2");
        $sqlEmployes->execute();
    ...
    <?php
        $sqlEmployes = $db->prepare("SELECT u.id AS idUser, u.last_name AS lastNAME, u.first_name AS firstName, u.phone as phone, u.email AS email, u.adress AS adress, uc.id AS idCategory, uc.label AS nameCategory FROM user u INNER JOIN user_category uc ON u.user_category_id = uc.id  WHERE u.id = 3");
        $sqlEmployes->execute();
        ...
        while($sqlEmploye = $sqlEmployes->fetch()) {
            ...
            echo "<tr>
                    ...
                    <td>
                        ...
                        <button onclick='editClient($id, \"$lastName\", \"$firstName\", \"$phone\", \"$email\", \"$adress\", $id_category)'><i class='hgi hgi-stroke hgi-file-edit'></i></button>
                    </td>
                </tr>";
        }
    ?>
